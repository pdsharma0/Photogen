...
bool MeshComponentManager::AddEntity(Entity e, Mesh* m) {
	...
	if (AddEntityToComponent(_currIndex, _data.n, e, _map)) {
		...
		_data.mesh[index] = m; // Just store a pointer to the entity's mesh
		return true;
	}
...
void MeshComponentManager::MoveEntityData(Entity e, unsigned src, unsigned dst) {
...
void MeshComponentManager::RemoveEntity(Entity e) {
	RemoveEntityFromComponent(_currIndex, e, _map, std::bind(&MeshComponentManager::MoveEntityData, this, e, std::placeholders::_1, std::placeholders::_2));
}
